...
| Calling a host-side helper inside a `#[cube] fn` body: macro tries to rewrite as `helper::expand(scope, …)` and errors with "is not a crate or module". | Wrap in `comptime!()`: `Array::<u32>::from_data(comptime!(my_host_fn()))`. Tells the macro the expression is regular Rust, evaluated at IR build time. |
| `&mut <CubeType>` parameter mutation works only when **all fields are `CubePrimitive`** (u32, bool, etc.). With CubeType-typed fields (e.g. `Ext4`) the macro errors with `From<…Expand>` trait-bound failures referencing unrelated cubecl primitives like `Shared<Barrier>`. | Use **value-in / value-out**: take the struct by value, return the updated struct, caller threads via `let cstate = helper(cstate);` shadowing. |
| `let mut cs: <CubeType-with-CubeType-fields> = …; cs = helper(cs);` reassignment hits the same limitation as `&mut`. (A `let mut` of a `CubePrimitive` local — `u32`, `bool`, etc. — is fine. Only CubeType-with-CubeType-fields trips it.) | For loops where you need to thread a CubeType accumulator: use a **comptime-recursive `#[cube] fn`** — `if comptime!(i >= N) { acc } else { let acc = step(acc); recurse(acc, comptime!(i + 1)) }`. cubecl resolves at IR build time, generating an inlined chain of N calls. Pattern in `r0-poseidon1/src/constraint.rs` (`full_round_chain`, `partial_chain`). |
| Runtime u32 kernel arguments need wrapping. | Use `ScalarArg::new(value)` at the launch site for any non-comptime u32 parameter. |
| Local `Array::<u32>::new(16)` for per-thread register-resident state lowers to `var a_0: array<u32, 16>` in WGSL — function-scope, scalarizes with comptime indices. (Confirmed end-to-end on Mac wgpu by `r0-poseidon1`'s 16-slot working set.) | Use `Array::<u32>::new(comptime!(N))` inside a `#[cube] fn`; index with `state[comptime!(i as usize)]`. Cleaner than `Line<u32>` of size N for pure local storage. |
| `Array::<u32>::from_data(comptime!(host_fn()))` lowers to a WGSL module-scope `const arrays_N: array<u32, M> = array(...)`. Reads with constant indices fold to literals downstream. | Use this for compile-time-fixed tables (round constants, FFT lambdas, partial-round weights, etc.) — no device-side buffer needed. `r0-poseidon1` bakes ~7 KiB of constants this way per kernel. |
