...
| `let mut cs: <CubeType-with-CubeType-fields> = …; cs = helper(cs);` reassignment hits the same limitation as `&mut`. | For loops where you need to thread a CubeType accumulator: use a **comptime-recursive `#[cube] fn`** — `if comptime!(i >= N) { acc } else { let acc = step(acc); recurse(acc, comptime!(i + 1)) }`. cubecl resolves at IR build time, generating an inlined chain of N calls. Pattern in `r0-poseidon1/src/constraint.rs` (`full_round_chain`, `partial_chain`). |
| Runtime u32 kernel arguments need wrapping. | Use `ScalarArg::new(value)` at the launch site for any non-comptime u32 parameter. |
| Local `Array::<u32>::new(16)` for per-thread register-resident state lowers to `var a_0: array<u32, 16>` in WGSL — function-scope, scalarizes with comptime indices. Verified scalarization in `r0-cube/tests/array16_spike.rs`. | Use `Array::<u32>::new(comptime!(N))` inside a `#[cube] fn`; index with `state[comptime!(i as usize)]`. Cleaner than `Line<u32>` of size N for pure local storage. |
| `Array::<u32>::from_data(comptime!(host_fn()))` lowers to a WGSL module-scope `const arrays_N: array<u32, M> = array(...)`. Reads with constant indices fold to literals downstream. | Use this for compile-time-fixed tables (round constants, FFT lambdas, etc.) — no device-side buffer needed. |
